<template>
  <section class="home-categories">
    <h2 class="home-categories__title">Meubles</h2>
    <div class="home-categories__container">
      <ProductItem
        v-for="ProductItem in homeFurnitureProducts"
    ...
    <div class="home-categories__link-container">
      <router-link :to="`/categorie/4`" class="home-categories__link"
        >Voir tous les meubles</router-link
      >
    </div>
  </section>
</template>
...
export default {
  ...
  mounted: function () {
    ApiClient.get("/wp/v2/products?per_page=4&category=4").then((response) => {
      this.homeFurnitureProducts = response.data;
    });
  },
};
</script>
